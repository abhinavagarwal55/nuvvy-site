...
const ITEM_STATUS_BADGE: Record<string, { cls: string; label: string }> = {
  pending: { cls: "bg-stone/20 text-charcoal", label: "Pending" },
  on_trip: { cls: "bg-blue-50 text-blue-700", label: "On trip" },
  ...
  const [state, setState] = useState<Record<string, PlantProcureState>>(() => {
    ...
    for (const row of rows) {
      const procurableItems = row.items.filter(
        (i) => i.status === "on_trip" || i.status === "procured"
      );
    ...
    try {
      ...
      for (const row of rows) {
        ...
        for (const item of procurableItems) {
          ...
          if (allocation === 0 && item.status === "on_trip") {
            // Skip items with zero allocation — they keep their current status
            continue;
          }

          // Only call API for on_trip items, or procured items being re-edited
          if (item.status !== "on_trip" && item.status !== "procured") continue;
...
export default function NurseryTripDetailPage() {
  ...
  async function openAddOrders() {
    ...
    try {
      // Pending items are eligible to join a trip — they can belong to an order
      // in any pipeline state, so we fetch all orders and filter by item status.
      const res = await fetch("/api/ops/plant-orders?include_items=true");
      if (res.ok) {
        const json = await res.json();
                      ...
                      {group.items.map((item) => {
                        ...
                        return (
                          <div key={item.id} className="border-t border-stone/30 pt-2 first:border-0 first:pt-0">
                            ...
                            {isCompleted && item.status === "on_trip" && (
                              <button
        ...
        <div className="px-4 pt-4 max-w-[700px] mx-auto">
          <div className="bg-offwhite rounded-2xl border border-stone/60 p-4">
            ...
            {requestedLoading ? (
              ...
              <p className="text-sm text-stone text-center py-6">No pending items available.</p>
            ) : (
              <>
                {/* Select All */}
