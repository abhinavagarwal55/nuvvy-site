...
import { Pencil, Check, X, Loader2, AlertTriangle, ArrowLeft, Languages } from "lucide-react";
...
export default function CareActionsSettingsPage() {
  ...
  const [txPromptId, setTxPromptId] = useState<string | null>(null);
  ...
  function setTxField(id: string, field: keyof CareActionType, value: string) {
  ...
  async function saveTx(
    ...
    await load();
    if (opts.promptAfter) setTxPromptId(t.id);
    else if (txPromptId === t.id) setTxPromptId(null);
  }
  ...
  return (
    <div className="min-h-screen bg-cream pb-24 md:pl-56">
      ...
      <div className="px-4 md:px-8 pt-6 max-w-[720px] space-y-6">
        ...
        {/* Names & translations (English display + hi/kn). English display is
            admin-only; hi/kn are editable by admin + horticulturist. */}
        <section>
          <h2
          ...
          <p className="text-xs text-sage mb-3">
            {isAdmin
              ? "The name gardeners see, with Hindi and Kannada translations."
              ...
              <div key={t.id} className="bg-offwhite rounded-2xl border border-stone/60 p-4 space-y-3">
                ...
                <div className="grid gap-3 md:grid-cols-3">
                  <label className="block">
                    ...
                    <input
                      className="w-full mt-1 px-3 py-2 border border-stone rounded-xl text-sm text-charcoal bg-offwhite focus:outline-none focus:border-forest"
                      value={t.display_name ?? ""}
                      disabled={!isAdmin}
                      placeholder="—"
                      onChange={(e) => setTxField(t.id, "display_name", e.target.value)}
                      onBlur={() => {
                        if (isAdmin && (t.display_name ?? "").trim())
                          saveTx(t, { display_name: t.display_name }, { promptAfter: true });
                      }}
                    />
                  </label>
                  <label className="block">
                    ...
                    <input
                      ...
                      placeholder="—"
                      onChange={(e) => setTxField(t.id, "display_name_hi", e.target.value)}
                      onBlur={() => saveTx(t, { display_name_hi: t.display_name_hi || null })}
                    />
                  </label>
                  <label className="block">
                    ...
                    <input
                      ...
                      placeholder="—"
                      onChange={(e) => setTxField(t.id, "display_name_kn", e.target.value)}
                      onBlur={() => saveTx(t, { display_name_kn: t.display_name_kn || null })}
                    />
                  </label>
                </div>
                {txPromptId === t.id && needsTx(t) && (
